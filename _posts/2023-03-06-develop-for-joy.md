...
Wearing a team lead hat developer happiness becomes the most important aspect. I always hire people smarter than me and care about their well-being. Contract work is a constant pressure. So one of the goals is to keep the happiness baseline from dipping
...
## Small tasks
We estimate tasks to know how much we can do, and to not take too much load. I like to use points after Bob Martin's idea. `3` is a "standard task". A "Standard task" is you can wrap up before lunch. We use `0`, `1`, `2`, `3`
 and `5` story points relative to the standard task:

- ` 0 ` – I'll close it right away (0~15 min)
- ` 1 ` - Won't take more that an hour
...
[A more strict](https://blog.borodutch.com/sup-simplified-scrum-for-your-development-team/) version of the same idea is the task can be of any size, but in the sprint backlog, you can have only 1's. Also see [Microtasking](https://www.yegor256.com/2017/11/28/microtasking.html)
...
* Convert it to an Epic with a checklist of subtasks. Subtasks should be closed in order for this Epic to be considered complete.
...
## Let dev go their own way
The flow is reachable when people work on something they want to work on the most. Chop tickets so everyone could find jobs they like best. Somebody loves data science, or frontend, or testing. Have a ticket for everything.

## Go together
Then let people work in small teams of two or three people on an Epic. That will improve everything. Morale, speed, efficiency. That works incredibly cool when the team is spread across different time zones. The work continues when you are sleeping. That helps to train communication skills and fill in the blanks for the people working with it.
...
## Automate everything
Before we had to write our own automation now github actions seem to do everything. Use it. The project should embrace the code developer produces. Test it, warn about potential issues, assign the reviewers, assign labels, status checks, everything. Whatever your process is - there is a bot just for that.
...
If you do not agree with something please discuss it in public discussion. Focus on important stuff, see [Bikeshedding](http://en.wiktionary.org/wiki/bikeshedding)

## Post metrics
Metrics should be a fact of life. When I had an edication project we had teacher leaderboard in the office and student leaderboards in school. No pressure just everyone could see how they are doing. Scrum corkboard on the wall with the paper notes that we moved when the task changed state. How the things are going was al obvious at the glance.

## Green builds
Do not send mixed signals. Every developer waits for green. If you see some build step is always failing, either change it to pending or remove it. There should be a positive reinforcement. Plus it's annoying when you have to check the build status manually every time.

## Simple branching
Before there was this craze with `flow`, `git-flow`, `github-flow`. People were foaming at their mouth how to name branches. I like [Trunk-based development](https://trunkbaseddevelopment.com/) with some simple branch naming rules like `0000-task-title`. The `master` branch is always ready to release and continuously deployed to a staging environment after every green build. Red build or a failing migration in `master` is an incident that should be fixed asap.
...
The first line should describe the commit as much as possible, so it's possible to do `git log --oneline` makes sense. [Conventional commits](https://www.conventionalcommits.org/) seem to work very well with the simple prefixes like `feat:` or `fix:`

## One feature – one commit
This makes rollbacks and cherry-picking a non-issue. Plus the history is almost linear, especially if you always rebase. The idea a bit controversial if you are constantly force-pushing and that makes code hard to review. So one could keep multiple commits until the branch is reviewed and squash on merge.

## Best pull request removes code
First rule of software quality:
## ` e = mc² `
*) ` errors = more code `
