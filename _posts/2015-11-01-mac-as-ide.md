...
where apps work together for the greater good.
...
I control eveything from keyboard: apps, windows. Is it faster and less
...
I use these two tiny apps from the same developer for additional keyboard tricks.
...
I don't minimize apps. I find this annoying. If there is too much clutter I
simply hide either current app via system shortcut `Cmd-H` or hide everything
else via `Cmd-Opt-H` (Hide Others). Some apps might need `Cmd-0` to show main window
after gaining focus back. If anything does not show up you can check in `Menu >
Window` for the given app.
...
BetterTouchTool also add window snapping akin to Windows' AeroSnap which is
pretty nice when you are feeling too lazy to reach the keyboard.
...
* `Cmd-Opt-Ctrl-~..5` — To rate music in iTunes

And `Space` which is much too powerful as it is also my leader key in Vim and
